    ...
    for(int period=1; period<=dimZ/(blockZ*nDevices)+1; period++){
        ...
        {   
            ...
            for(int file = 0; file < num_files; file++) {
                std::string filename = "frequency_10M_stride001s_sample2400_2/uvw" + std::to_string(file + 1) + "frequency10M.txt";
        ...
        for(int i = 0; i < nDevices; ++i) {
            ...
            // 计算满足条件的元素数量
            size_t count = thrust::count_if(h_freqArray.begin(), h_freqArray.end(), is_nonzero());

            // 写入文件中  验证正确
            std::ofstream outFile("uvw_frequency_50/uvwMap225_"+std::to_string(startZ)+"_"+std::to_string(startZ + blockZ)+".txt");
