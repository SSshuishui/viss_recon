...
__global__ void calculateOrbit(double* dM, double* x, double* y, double* z, double* Mt, int OrbitCounts, int OrbitRes, double d2r, double e, double a, double mu_e, double argp, double incl, int index, int sat) {
    // 计算全局索引
...
void MIncline(int index, double frequency, double stride, int gpu_id) {
    ...
    thrust::device_vector<double> xt(OrbitRes * OrbitCounts * satnum / 3, 0);
    thrust::device_vector<double> yt(OrbitRes * OrbitCounts * satnum / 3, 0);
    thrust::device_vector<double> zt(OrbitRes * OrbitCounts * satnum / 3, 0);
