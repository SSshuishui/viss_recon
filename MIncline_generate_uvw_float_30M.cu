...
__global__ void calculateOrbit_X(float* dM, float* x, float* Mt, long OrbitCounts, long OrbitRes, float d2r, float e, float a, float mu_e, float argp, float incl, int index, long sat) {
    // 计算全局索引
    long globalIdx = blockIdx.x * blockDim.x + threadIdx.x;
    // 计算g和k的值
    long k = globalIdx / OrbitRes;  // 圈数
    long po = globalIdx % OrbitRes;  // 点数
    if(k < OrbitCounts && po < OrbitRes) {
        ...
        float w = theta + argp;
        long tmp = (sat*OrbitRes*OrbitCounts) + k * OrbitRes + po;
        x[tmp] = r * (cos(w) * cos(raan) - sin(w) * cos(incl) * sin(raan));
    }
}
__global__ void calculateOrbit_Y(float* dM, float* y, float* Mt, long OrbitCounts, long OrbitRes, float d2r, float e, float a, float mu_e, float argp, float incl, int index, long sat) {
    // 计算全局索引
    long globalIdx = blockIdx.x * blockDim.x + threadIdx.x;
    // 计算g和k的值
    long k = globalIdx / OrbitRes;  // 圈数
...
__global__ void calculateOrbit_Z(float* dM, float* z, float* Mt, long OrbitCounts, long OrbitRes, float d2r, float e, float a, float mu_e, float argp, float incl, int index, long sat) {
    // 计算全局索引
    long globalIdx = blockIdx.x * blockDim.x + threadIdx.x;
    // 计算g和k的值
    long k = globalIdx / OrbitRes;  // 圈数
...
__global__ void uvwPosition_X(float* x, float* xt, long OrbitCounts, long OrbitRes, long interval, long span, long sat) {
    long globalIdx = blockIdx.x * blockDim.x + threadIdx.x;   // OrbitCounts * OrbitRes / 3  大小和xt yt zt的维度一样
    long k = globalIdx / (OrbitRes/3) + 1;  // 第几圈

    long offset = 0;
    long tmp = sat*OrbitRes*OrbitCounts;  // 计算是用的x的哪一段
    if(k <= OrbitCounts){
        offset = (k-1)*interval;
        long endIndexWithoutMin = (k-1)*OrbitRes + offset + span;
        long startIdx = (k-1)*OrbitRes + offset; 
        if ((k-1)*(OrbitRes+interval) > OrbitRes*OrbitCounts) {
            long reset_index = (OrbitRes*OrbitCounts) / (OrbitRes+interval) + 1;
            // 当超出范围之后，从头开始采样，但是偏置还是存在
            startIdx = (k-1-reset_index)*OrbitRes + offset;
            endIndexWithoutMin = (k-1-reset_index)*OrbitRes + offset + span;
        }

        long circleIdx = startIdx + (globalIdx-(k-1)*span);  // 每一圈中对应的向量下标
        // 超出的部分就设置为0
...
__global__ void uvwPosition_Y(float* y, float* yt, long OrbitCounts, long OrbitRes, long interval, long span, long sat) {
    long globalIdx = blockIdx.x * blockDim.x + threadIdx.x;   // OrbitCounts * OrbitRes / 3  大小和xt yt zt的维度一样
    long k = globalIdx / (OrbitRes/3) + 1;  // 第几圈
...
__global__ void uvwPosition_Z(float* z, float* zt, long OrbitCounts, long OrbitRes, long interval, long span, long sat) {
    long globalIdx = blockIdx.x * blockDim.x + threadIdx.x;   // OrbitCounts * OrbitRes / 3  大小和xt yt zt的维度一样
    long k = globalIdx / (OrbitRes/3) + 1;  // 第几圈
...
void MIncline(int index, float frequency, float stride, int gpu_id) {
    long OrbitRes;
    if(frequency == 3e7){
        OrbitRes = 2.4 * 3600 * 300;
    }else{
        OrbitRes = 2.4 * 3600 * int(1.0/stride);
    }

    long OrbitCounts = 140;
    long interval = OrbitRes / 360 * 10;  
    long span = OrbitRes / 3; 
    long lambda = 3e8/frequency;
    ...
    for (int ss = 0; ss < satnum; ss++) 
    {
        ...
        CHECK(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &posBlockSize, calculateOrbit_X, 0, 0));
        posGridSize = floor(OrbitCounts * OrbitRes + posBlockSize - 1) / posBlockSize;
        std::cout << "GPU " << gpu_id << " Calculate position X, blockSize: " << posBlockSize << std::endl;
        std::cout << "GPU " << gpu_id << " Calculate position X, girdSize: " << posGridSize << std::endl;
        calculateOrbit_X<<<posGridSize, posBlockSize>>>(
        ...
        std::cout << "GPU " << gpu_id << " calculateOrbit X compute success!" << std::endl;
        // 记录position结束事件
        ...
        printf("Frequency-%f Index-%d Sat-%d On GPU-%d Calculate position X Cost Time is: %f s\n", frequency, index, ss+1, gpu_id, posMS/1000);
        std::cout << "sat " << ss+1 << " position X compute successe! on GPU " << gpu_id << std::endl;
        ...
        CHECK(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &uvwBlockSize, uvwPosition_X, 0, 0));
        uvwGridSize = floor(OrbitCounts * OrbitRes / 3 + uvwBlockSize - 1) / uvwBlockSize;
        std::cout << "GPU " << gpu_id << " U position, blockSize: " << uvwBlockSize << std::endl;
        std::cout << "GPU " << gpu_id << " U position, girdSize: " << uvwGridSize << std::endl;
        uvwPosition_X<<<uvwGridSize, uvwBlockSize>>>(
            thrust::raw_pointer_cast(x.data()),
            thrust::raw_pointer_cast(xt.data()),
            OrbitCounts, OrbitRes, interval, span, ss
        );
        std::cout << "GPU " << gpu_id << " U Position compute success!" << std::endl;
        // 记录uvw结束事件
        ...
        printf("Frequency-%f Index-%d Sat-%d On GPU-%d Compute xt Cost Time is: %f s\n", frequency, index, ss+1, gpu_id, uvwMS/1000);
        std::cout << "Will Save! Sat " << ss+1 << " U position compute successe! on GPU " << gpu_id << std::endl;
        CHECK(cudaDeviceSynchronize());  // 线程同步，等前面完成了再进行下一步 
    }
    // 用完xyz后，释放它们的内存
    x.clear();
    x.shrink_to_fit();
    std::cout << "GPU " << gpu_id << " X compute successe! " << std::endl;
    ...
    for (int ss = 0; ss < satnum; ss++) 
    {
        ...
        CHECK(cudaEventCreate(&posstop));
        CHECK(cudaEventRecord(posstart));
        CHECK(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &posBlockSize, calculateOrbit_Y, 0, 0));
        posGridSize = floor(OrbitCounts * OrbitRes + posBlockSize - 1) / posBlockSize;
        std::cout << "GPU " << gpu_id << " Calculate position Y, blockSize: " << posBlockSize << std::endl;
        std::cout << "GPU " << gpu_id << " Calculate position Y, girdSize: " << posGridSize << std::endl;
        calculateOrbit_Y<<<posGridSize, posBlockSize>>>(
            thrust::raw_pointer_cast(dM.data()),
        ...
        std::cout << "GPU " << gpu_id << " calculateOrbit Y compute success!" << std::endl;
        // 记录position结束事件
        CHECK(cudaEventRecord(posstop));
        CHECK(cudaEventSynchronize(posstop));
        // 计算经过的时间
        float posMS = 0;
        CHECK(cudaEventElapsedTime(&posMS, posstart, posstop));
        printf("Frequency-%f Index-%d Sat-%d On GPU-%d Calculate position Y Cost Time is: %f s\n", frequency, index, ss+1, gpu_id, posMS/1000);
        std::cout << "sat " << ss+1 << " position Y compute successe! on GPU " << gpu_id << std::endl;
        CHECK(cudaDeviceSynchronize());  // 线程同步，等前面完成了再进行下一步    
        ...
        int uvwBlockSize;
        int uvwGridSize;    // 实际网格大小
        CHECK(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &uvwBlockSize, uvwPosition_Y, 0, 0));
        uvwGridSize = floor(OrbitCounts * OrbitRes / 3 + uvwBlockSize - 1) / uvwBlockSize;
        std::cout << "GPU " << gpu_id << " V position, blockSize: " << uvwBlockSize << std::endl;
        std::cout << "GPU " << gpu_id << " V position, girdSize: " << uvwGridSize << std::endl;
        uvwPosition_Y<<<uvwGridSize, uvwBlockSize>>>(
            thrust::raw_pointer_cast(y.data()),
            thrust::raw_pointer_cast(yt.data()),
            OrbitCounts, OrbitRes, interval, span, ss
        );
        std::cout << "GPU " << gpu_id << " V Position compute success!" << std::endl;
        // 记录uvw结束事件
        CHECK(cudaEventRecord(uvwstop));
        CHECK(cudaEventSynchronize(uvwstop));
        // 计算经过的时间
        float uvwMS = 0;
        CHECK(cudaEventElapsedTime(&uvwMS, uvwstart, uvwstop));
        printf("Frequency-%f Index-%d Sat-%d On GPU-%d Compute yt Cost Time is: %f s\n", frequency, index, ss+1, gpu_id, uvwMS/1000);
        std::cout << "Will Save! Sat " << ss+1 << " V position compute successe! on GPU " << gpu_id << std::endl;
        CHECK(cudaDeviceSynchronize());  // 线程同步，等前面完成了再进行下一步 
    }
    // 用完xyz后，释放它们的内存
    y.clear();
    ...
    for (int ss = 0; ss < satnum; ss++) 
    {
        ...
        CHECK(cudaEventCreate(&posstop));
        CHECK(cudaEventRecord(posstart));
        CHECK(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &posBlockSize, calculateOrbit_Z, 0, 0));
        posGridSize = floor(OrbitCounts * OrbitRes + posBlockSize - 1) / posBlockSize;
        std::cout << "GPU " << gpu_id << " Calculate position Z, blockSize: " << posBlockSize << std::endl;
        std::cout << "GPU " << gpu_id << " Calculate position Z, girdSize: " << posGridSize << std::endl;
        calculateOrbit_Z<<<posGridSize, posBlockSize>>>(
            thrust::raw_pointer_cast(dM.data()),
        ...
        std::cout << "GPU " << gpu_id << " calculateOrbit Z compute success!" << std::endl;
        // 记录position结束事件
        CHECK(cudaEventRecord(posstop));
        CHECK(cudaEventSynchronize(posstop));
        // 计算经过的时间
        float posMS = 0;
        CHECK(cudaEventElapsedTime(&posMS, posstart, posstop));
        printf("Frequency-%f Index-%d Sat-%d On GPU-%d Calculate position Z Cost Time is: %f s\n", frequency, index, ss+1, gpu_id, posMS/1000);
        std::cout << "sat " << ss+1 << " position Z compute successe! on GPU " << gpu_id << std::endl;
        CHECK(cudaDeviceSynchronize());  // 线程同步，等前面完成了再进行下一步    
        ...
        int uvwBlockSize;
        int uvwGridSize;    // 实际网格大小
        CHECK(cudaOccupancyMaxPotentialBlockSize(&minGridSize, &uvwBlockSize, uvwPosition_Z, 0, 0));
        uvwGridSize = floor(OrbitCounts * OrbitRes / 3 + uvwBlockSize - 1) / uvwBlockSize;
        std::cout << "GPU " << gpu_id << " W position, blockSize: " << uvwBlockSize << std::endl;
        std::cout << "GPU " << gpu_id << " W position, girdSize: " << uvwGridSize << std::endl;
        uvwPosition_X<<<uvwGridSize, uvwBlockSize>>>(
            thrust::raw_pointer_cast(z.data()),
            thrust::raw_pointer_cast(zt.data()),
            OrbitCounts, OrbitRes, interval, span, ss
        );
        std::cout << "GPU " << gpu_id << " W Position compute success!" << std::endl;
        // 记录uvw结束事件
        CHECK(cudaEventRecord(uvwstop));
        CHECK(cudaEventSynchronize(uvwstop));
        // 计算经过的时间
        float uvwMS = 0;
        CHECK(cudaEventElapsedTime(&uvwMS, uvwstart, uvwstop));
        printf("Frequency-%f Index-%d Sat-%d On GPU-%d Compute zt Cost Time is: %f s\n", frequency, index, ss+1, gpu_id, uvwMS/1000);
        std::cout << "Will Save! Sat " << ss+1 << " W position compute successe! on GPU " << gpu_id << std::endl;
        CHECK(cudaDeviceSynchronize());  // 线程同步，等前面完成了再进行下一步 
    }
    // 用完xyz后，释放它们的内存
    z.clear();
    ...
    fname << "frequency_30M/uvw" << index << "frequency30M.txt";
