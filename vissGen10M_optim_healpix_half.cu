...
string para;
string duration = "frequency10M_half";  // 第几个周期的uvw
string sufix = ".txt";

// 10 M
const int uvw_presize = 29400000;  
    ...
    #pragma omp parallel
    {
        ...
        for (int p = tid+start_period; p < days; p += nDevices) 
        {
            ...
            {   
                ...
                std::ofstream F_File;
                string address_F = "F_recon_10M/F" + to_string(p+1) + "_healpix_fromB_changeMin.txt";
...
int main()
{   
    int start_period = 24;  // 从哪个周期开始，一共是130个周期
