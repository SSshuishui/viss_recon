...
int main(int argc, char **argv) {
    ...
    int dimX = 19938, dimY = 19938, dimZ = 10002;
    ...
    for(int period=1; period<=dimZ/(blockZ*nDevices)+1; period++){
        ...
        {   
            ...
            for(int file = 0; file < num_files; file++) {
                std::string filename = "frequency_30M/uvw" + std::to_string(file + 1) + "frequency30M.txt";
        ...
        for(int i = 0; i < nDevices; ++i) {
            ...
            // 写入文件中  验证正确
            std::ofstream outFile("frequency_30M/uvw_seg/uvwMap130_"+std::to_string(startZ)+"_"+std::to_string(startZ + blockZ)+".txt");
