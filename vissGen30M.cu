...
{   
    ...
    string computeC_dir = "F_recon_30M/computeC.txt";
    ...
    #pragma omp parallel
    {
        ...
        for (int p = tid + id*amount/totalnode; p < (id + 1) * amount / totalnode; p += nDevices) 
        {
            ...
            #pragma omp critical
            {
                ...
                if (uvwFile.is_open()) {
                    while (uvwFile >> u_point >> v_point >> w_point >> freq_point) {
                        ...
                        uvwMapVector[uvw_index] = Complex(1 / freq_point, 0);
                        uvw_index++;
                    }
                }               
            ...
            if(p==0){
                writeToFile(F, "F_recon_30M/F30M.txt");
            }
