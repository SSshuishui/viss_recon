...
#     filename = f"torch_10M/viss2_{index}.csv"
#     df = pd.read_csv(filename)
#     combined_df = pd.concat([combined_df, df], ignore_index=True)

# # 将合并后的 DataFrame 保存为一个新的 CSV 文件
# combined_df.to_csv(f"torch_10M/viss2.csv", index=False)
...
# 遍历所有CSV文件并累计行数
for index in range(18):
    filename = f"{directory}/viss2_{index}.csv"
    # 检查文件是否存在
...
combined_filename = f"{directory}/viss2.csv"
